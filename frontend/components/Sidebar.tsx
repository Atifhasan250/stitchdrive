...
const navItems = [
  {
    ...
    label: "Overview",
    icon: (active: boolean) => (
      <svg className="h-4 w-4 flex-shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={active ? 2.5 : 1.75}>
...
export default function Sidebar({ isOpen, onClose }: SidebarProps) {
  ...
  useEffect(() => {
    if (localStorage.getItem("sd-sidebar-collapsed") === "true") setCollapsed(true);
  }, []);
  ...
  return (
    <>
      ...
      <aside
        className={`
          fixed inset-y-0 left-0 z-40 flex h-screen flex-none flex-col
          border-r border-sd-border bg-sd-sidebar
      ...
      >
        ...
        <div className={`flex items-center border-b border-sd-border px-3 py-[13px] ${collapsed ? "lg:justify-center" : "justify-between"}`}>
          ...
          <button
            onClick={onClose}
            className="flex h-6 w-6 items-center justify-center rounded-md text-sd-text3 hover:bg-sd-s2 hover:text-sd-text transition lg:hidden"
          >
          ...
          {!collapsed && (
            <button
              ...
              className="hidden h-6 w-6 items-center justify-center rounded-md text-sd-text3 hover:bg-sd-s2 hover:text-sd-text transition lg:flex"
            >
              <svg className="h-3.5 w-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
        ...
        {/* Nav */}
        <nav className={`flex-1 space-y-0.5 py-3 ${collapsed ? "lg:px-2 px-2" : "px-2"}`}>
          {!collapsed && (
            <p className="mb-1.5 px-2 text-[10px] font-semibold uppercase tracking-widest text-sd-text3">
              Navigation
            </p>
          )}
          {navItems.map((item) => {
            ...
            return (
              <Link
                ...
                className={`
                  ...
                  ${collapsed ? "lg:justify-center lg:px-0 lg:py-2.5 px-2 py-2.5" : "px-2 py-2.5"}
                  ${isActive
                    ? "nav-active text-sd-accent font-medium"
                    : "text-sd-text2 hover:bg-sd-s2 hover:text-sd-text"
                  }
                `}
              >
        ...
        {/* Storage widget */}
        {!collapsed && (
          <div className="mx-2 mb-3 rounded-xl border border-sd-border bg-sd-s2 p-3.5">
            <div className="mb-2.5 flex items-center justify-between">
              <span className="text-xs font-medium text-sd-text">Storage Pool</span>
              <span className="rounded-full border border-sd-border px-2 py-0.5 text-[10px] font-medium text-sd-text3">
                {connectedCount} drive{connectedCount !== 1 ? "s" : ""}
              </span>
            </div>
            <div className="mb-2 h-1.5 w-full overflow-hidden rounded-full bg-sd-s3">
              <div
                className="progress-bar h-full rounded-full transition-all duration-700"
                style={{ width: `${pct}%` }}
              />
            </div>
            <div className="flex justify-between text-[11px] text-sd-text3">
